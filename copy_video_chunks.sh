...
# copy_video_chunks.sh
#
# Step 1 of video processing pipeline: Copy and organize video chunks
#
# This script handles ONLY the copying and basic organization of Rewind chunks:
# - Copies chunks to desktop for safe testing
# - Converts to MP4 format with readable timestamp naming
# - Preserves original metadata
# - Organizes by date directories
#
# This is followed by: detect_meetings.sh -> extract_text_from_videos.sh
...
print_summary() {
    ...
    if [ $error_count -gt 0 ]; then
        ...
        log "✅ Copying and organizing completed successfully!"
    fi

    log ""
    log "📋 Next Steps:"
    log "1. Run './detect_meetings.sh' to organize by meeting/screenshot"
    log "2. Run './extract_text_from_videos.sh' to extract text via OCR"
}

# Main execution
main() {
    log "Starting video chunk copying and organization..."
